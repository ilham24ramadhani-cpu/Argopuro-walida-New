// Data hasil produksi (menggunakan API Service dengan fallback ke localStorage)
let hasilProduksi = [];
let currentEditId = null;
let currentDeleteId = null;

// Load data hasil produksi dari API atau localStorage
async function loadHasilProduksiData() {
  try {
    if (window.API && window.API.HasilProduksi) {
      ...
      // Inisialisasi data contoh hanya jika localStorage benar-benar null
      if (hasilProduksi.length === 0 && !localStorage.getItem("hasilProduksi")) {
        hasilProduksi = [
          {
            id: 1,
            tipeProduk: "Kopi Sangrai",
            kemasan: "500 gram",
            jenisKopi: "Arabika",
            prosesPengolahan: "Washed Process",
            levelRoasting: "Medium Roast",
            tanggal: "2024-01-25",
            beratSaatIni: 25,
            jumlah: 50,
          },
          {
            id: 2,
            tipeProduk: "Kopi Bubuk",
            kemasan: "250 gram",
            jenisKopi: "Robusta",
            prosesPengolahan: "Natural Process",
            levelRoasting: "Dark Roast",
            tanggal: "2024-01-26",
            beratSaatIni: 25,
            jumlah: 100,
          },
    ...
    console.error("Error loading hasil produksi:", error);
    hasilProduksi = JSON.parse(localStorage.getItem("hasilProduksi") || "[]");
  }
}
...
async function loadProduksiData() {
  ...
  // Jika tidak ada di dataset, cari dari API atau localStorage berdasarkan idProduksi (string)
  if (!produksiData) {
    try {
      if (window.API && window.API.Produksi) {
        ...
        produksiData = produksi.find((p) => p.idProduksi === idProduksiValue);
        if (!produksiData && !isNaN(parseInt(idProduksiValue))) {
          produksiData = produksi.find((p) => p.id === parseInt(idProduksiValue));
        }
      }
    } catch (error) {
  ...
  try {
    if (window.API && window.API.Produksi) {
      ...
      if (hasilProduksiIdEdit) {
        ...
        const currentItem = hasilProduksi.find(h => h.id === parseInt(hasilProduksiIdEdit));
        if (currentItem) {
      ...
      const totalDikemas = hasilProduksi
        .filter(
          (h) =>
            ...
            h.id !== parseInt(hasilProduksiIdEdit || 0)
        )
        .reduce((sum, h) => sum + (parseFloat(h.beratSaatIni) || 0), 0);
      sisaTersedia = Math.max(0, beratAkhir - totalDikemas);
    }
  } catch (error) {
  ...
  try {
    if (window.API && window.API.Bahan && produksiData.idBahan) {
      ...
      const bahanTerpilih = bahan.find((b) => b.idBahan === produksiData.idBahan);
      jenisKopi = bahanTerpilih ? (bahanTerpilih.jenisKopi || "Arabika") : "Arabika";
      console.log("✅ Jenis Kopi dari localStorage:", jenisKopi);
    }
  } catch (error) {
  ...
  // Helper function untuk set value dengan retry
  const setSelectValue = (selectElement, value, fieldName) => {
    ...
    if (selectElement.value === value) {
      console.log(`✅ ${fieldName} berhasil di-set:`, value);
      selectElement.dispatchEvent(new Event("change", { bubbles: true }));
      return true;
    }
    
    // Jika tidak berhasil, coba cari dengan case-insensitive atau partial match
    ...
    const matchedOption = options.find(opt => {
      ...
      return optValue.toLowerCase() === searchValue.toLowerCase() ||
             optText.toLowerCase() === searchValue.toLowerCase() ||
             optValue === searchValue ||
             optText === searchValue;
    });
    
    if (matchedOption) {
      selectElement.value = matchedOption.value;
      console.log(`✅ ${fieldName} berhasil di-set dengan matching:`, matchedOption.value);
      selectElement.dispatchEvent(new Event("change", { bubbles: true }));
      return true;
    }
    
    console.warn(`⚠️ ${fieldName} tidak ditemukan di options. Value:`, value);
    console.warn(`   Available options:`, options.map(o => ({ value: o.value, text: o.textContent })));
    return false;
  };

  // Auto-fill Proses Pengolahan
  if (prosesPengolahanSelect) {
    const prosesValue = (produksiData.prosesPengolahan || "").toString().trim();
    console.log("🔄 Mengisi Proses Pengolahan dengan nilai:", prosesValue);
    
    if (prosesValue) {
      const success = setSelectValue(prosesPengolahanSelect, prosesValue, "Proses Pengolahan");

      // Jika tidak berhasil, coba lagi setelah delay (untuk handle race condition)
      if (!success) {
      setTimeout(() => {
          const retrySuccess = setSelectValue(prosesPengolahanSelect, prosesValue, "Proses Pengolahan (retry)");
          if (!retrySuccess) {
            console.error("❌ Proses Pengolahan gagal di-set setelah retry:", {
            expected: prosesValue,
              availableOptions: Array.from(prosesPengolahanSelect.options).map(o => ({
                value: o.value,
                text: o.textContent
              }))
            });
      ...
      console.warn("⚠️ Proses Pengolahan dari produksi kosong");
    }
  } else {
  ...
  if (jenisKopiSelect) {
    const jenisKopiValue = jenisKopi.toString().trim();
    console.log("🔄 Mengisi Jenis Kopi dengan nilai:", jenisKopiValue);
    
    if (jenisKopiValue) {
      const success = setSelectValue(jenisKopiSelect, jenisKopiValue, "Jenis Kopi");

      // Jika tidak berhasil, coba lagi setelah delay (untuk handle race condition)
      if (!success) {
      setTimeout(() => {
          const retrySuccess = setSelectValue(jenisKopiSelect, jenisKopiValue, "Jenis Kopi (retry)");
          if (!retrySuccess) {
            console.error("❌ Jenis Kopi gagal di-set setelah retry:", {
              expected: jenisKopiValue,
              availableOptions: Array.from(jenisKopiSelect.options).map(o => ({
                value: o.value,
                text: o.textContent
              }))
            });
      ...
      console.warn("⚠️ Jenis Kopi dari bahan kosong");
    }
  } else {
  ...
  // Auto-fill Tanggal
  if (tanggalInput) {
    const tanggalValue = produksiData.tanggalSekarang || new Date().toISOString().split("T")[0];
  ...
  console.log("✅ Data terkait otomatis terisi:", {
    ...
    jenisKopi: jenisKopi,
    tanggal: produksiData.tanggalSekarang || new Date().toISOString().split("T")[0],
  });
...
// terpilih agar user tahu batas maksimum proses pengemasan.
function updateSisaInfo(produksiIdOrIdProduksi, hasilProduksiIdEdit) {
  const produksi = JSON.parse(localStorage.getItem("produksi")) || [];
  const hasilProduksi = JSON.parse(localStorage.getItem("hasilProduksi")) || [];

  // Cari produksi berdasarkan idProduksi (string) terlebih dahulu, lalu ID (number) sebagai fallback
  let produksiData = produksi.find(
    (p) => p.idProduksi === produksiIdOrIdProduksi
  );
  if (!produksiData && !isNaN(parseInt(produksiIdOrIdProduksi))) {
    produksiData = produksi.find(
      (p) => p.id === parseInt(produksiIdOrIdProduksi)
    );
  }

  if (!produksiData || !produksiData.beratAkhir) return;
...
function calculateJumlahKemasan() {
  ...
  if (!kemasan || beratSaatIni <= 0) {
    jumlahInput.value = "";
    jumlahInput.setCustomValidity("Isi berat dan pilih kemasan terlebih dahulu");
    return;
  }
  ...
  if (ukuranKg > 0) {
    ...
    jumlahInput.value = "";
    jumlahInput.setCustomValidity("Ukuran kemasan tidak valid. Pastikan kemasan memiliki format yang benar (contoh: '250 gram' atau '5 kg')");
    jumlahInput.removeAttribute("title");
...
// [MASTER DATA → PRODUK] Mengisi dropdown tipe produk berdasarkan data master
// agar penamaan konsisten antar halaman.
async function loadProdukOptionsHasilProduksi() {
  try {
    ...
    if (window.API && window.API.MasterData && window.API.MasterData.produk) {
      ...
      dataProduk = JSON.parse(localStorage.getItem("dataProduk") || "[]");
    }
...
async function loadJenisKopiOptionsHasilProduksi() {
  try {
    let dataJenisKopi = [];
    if (window.API && window.API.MasterData && window.API.MasterData.jenisKopi) {
...
async function loadLevelRoastingOptions() {
  try {
    let dataRoasting = [];
    if (window.API && window.API.MasterData && window.API.MasterData.roasting) {
...
// pattern yang sama seperti di kelola_data.js
  ...
  try {
    ...
    if (window.API && window.API.MasterData && window.API.MasterData.kemasan) {
      try {
        ...
        console.log("[KEMASAN] Data kemasan dimuat dari API:", dataKemasanHasilProduksi.length, "items");
        
        // Simpan ke localStorage sebagai cache
        if (Array.isArray(dataKemasanHasilProduksi) && dataKemasanHasilProduksi.length > 0) {
          localStorage.setItem("dataKemasan", JSON.stringify(dataKemasanHasilProduksi));
        }
      } catch (apiError) {
        console.warn("⚠️ Error loading kemasan dari API, fallback ke localStorage:", apiError);
        // Fallback ke localStorage jika API error
        try {
          dataKemasanHasilProduksi = JSON.parse(localStorage.getItem("dataKemasan") || "[]");
        } catch (parseError) {
          console.error("Error loading dataKemasan dari localStorage:", parseError);
    ...
    const ukuranKemasan = dataKemasanHasilProduksi
      .map((item, index) => {
        ...
        console.warn(`[KEMASAN] Format item index ${index} tidak dikenal:`, item);
        return null;
      })
...
async function toggleRoastingField() {
  ...
  const needsRoasting =
    tipeProdukLower === "kopi sangrai" ||
    tipeProdukLower === "kopi bubuk" ||
    tipeProdukLower.includes("pixel"); // Partial match untuk "pixel(bahankopral)" atau variasi lainnya
...
async function editHasilProduksi(id) {
  ...
  document.getElementById("prosesPengolahan").value = h.prosesPengolahan || "";
...
async function saveHasilProduksi() {
  ...
  if (!idProduksiValue || idProduksiValue.trim() === "") {
    alert("ID Produksi WAJIB dipilih! Silakan pilih ID Produksi yang sudah masuk tahap Pengemasan.");
    if (idProduksiSelect) {
  ...
  // Jika masih tidak ada, cari dari API atau localStorage berdasarkan idProduksi (string)
  if (!produksiData) {
    try {
      if (window.API && window.API.Produksi) {
        produksiData = await window.API.Produksi.getById(idProduksiValue);
  ...
  if (!jumlah || jumlah <= 0) {
    ...
    calculateJumlahKemasan();
    const jumlahRecalculated = parseInt(document.getElementById("jumlah").value) || 0;
    
    if (jumlahRecalculated <= 0) {
      const beratSaatIni = parseFloat(document.getElementById("beratSaatIni").value) || 0;
      ...
      if (beratSaatIni > 0 && kemasanValue) {
        errorMessage += "⚠️ Periksa apakah ukuran kemasan valid atau berat mencukupi untuk kemasan yang dipilih.";
      }
  ...
  // Level roasting untuk Kopi Sangrai, Kopi Bubuk, dan Pixel (termasuk variasi seperti "pixel(bahankopral)")
  let levelRoasting = "";
  const needsRoasting =
    tipeProdukLower === "kopi sangrai" ||
    tipeProdukLower === "kopi bubuk" ||
  ...
  try {
    ...
    if (hasilProduksiId) {
      ...
      if (window.API && window.API.HasilProduksi) {
        ...
        // Fallback to localStorage
        const index = hasilProduksi.findIndex(
          (h) => h.id === parseInt(hasilProduksiId)
        );
        if (index !== -1) {
          hasilProduksi[index] = { ...hasilProduksi[index], ...hasilProduksiData };
          localStorage.setItem("hasilProduksi", JSON.stringify(hasilProduksi));
        }
      }
    } else {
      ...
      if (window.API && window.API.HasilProduksi) {
        await window.API.HasilProduksi.create(hasilProduksiData);
      } else {
        // Fallback to localStorage
        const newId =
          hasilProduksi.length > 0
            ...
            : 1;
        hasilProduksi.push({ id: newId, ...hasilProduksiData });
        localStorage.setItem("hasilProduksi", JSON.stringify(hasilProduksi));
      }
    }
  ...
  if (currentDeleteId) {
    try {
      ...
      if (window.API && window.API.HasilProduksi) {
        ...
        // Fallback to localStorage
        await loadHasilProduksiData();
        hasilProduksi = hasilProduksi.filter(
          (h) =>
            h.id !== currentDeleteId &&
            h._id !== currentDeleteId
        );
        localStorage.setItem("hasilProduksi", JSON.stringify(hasilProduksi));
      }
